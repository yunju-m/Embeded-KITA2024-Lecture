...
public class ChattingClient {
	...
	private Socket socket; // 서버와 송수신하기위한 클라이언트 소켓
	private BufferedReader in; // 서버에서 문자를 수신하는 스트림
	private PrintWriter out; // 서버에 문자를 송신하는 스트림
	...
	public ChattingClient(String serverAddress, int serverPort, Consumer<String> onMessageReceived) throws IOException {
		...
		// 클라이언트 출력스트림 (클라이언트 -> 서버), true: auto flush()
	...
	public void startClient() {
		new Thread(() -> {
			try {
				...
				// onMessageReceived에 line을 주고 수행
