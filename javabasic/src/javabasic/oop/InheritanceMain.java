...
public class InheritanceMain {
	...
	public static void main(String[] args) {
		...
		// child : GrandParent타입, 가지고 있는 참조는 Child
		// child2 : Child타입, 가지고 있는 참조는 Child (상위형변환에 의해서 Child 참조를 가지게 됨)
		Child child2 = (Child)child;
		System.out.println(child2.name + "님은 " + child2.age + "살이고 직업은 " + child2.job);

		// Overriding (오버라이딩 = 메소드재정의)
		GrandParent gp1 = new GrandParent("강할배", 90, "장군");
		...
		// 오버라이딩을 하는 이유
		// 상위타입으로 하위타입의 기능을 수정할 수 있도록 상위형변환을 수행한 것
		// 긱긱의 하위타입들을 상위타입으로 호출하면... 각각의 하위타입들을 알 필요가 없다.
